...
int sum(int zzz, int zz) 
{
	...
	sort(coup.begin(), coup.end(), happy_cmp);
	coup.erase(coup.begin()+(coup.size()-zz), coup.end());


    ////////////////////////////////////////////////////////////
	
	if(zzz > coup.size()) zzz = coup.size();
	cout << zzz-zz << " after break up most happiest couples are\n";
	cnt = 0;
	for( auto it = coup.begin(); cnt < zzz-zz && it != coup.end(); it++ ) {
		cnt++;
		cout << it->get_boy_name() << " " << it->get_girl_name() << " " << it->get_happiness() << endl;	
	}
	/*cout << zzz-zz << " after break up most compatible couples\n";
